...
fetch(url)
  ...
  .then(json => {
	...
	const htmlContentProject = `<div class="content">
        ...
        <p class="text"><i class="fa fa-circle-o"></i>${json.created_at}</p>
        <p class="text"><i class="fa fa-refresh"></i>${json.updated_at}</p>
      </div>`
	  
	  
	  let htmlContentCommunity = `<div class="content">
        <p class="text">
        ...
        <p class="text">
			...
			<a href="${json.contributors_url}" target="_blank">${json.open_issues_count}</a>				
			<span> contributors</span>
		</p>
      </div>`
	  
	  	let htmlContentCode = `<div class="content">
        <p class="text">
			<a href="${json.commits_url}" target="_blank" id="commit${json.name}"></a> 	
			<span>commits</span>
		</p>
...
getLanguages = (url, seletor) =>{
	fetch(url)
	  ...
	  .then(json => {
			...
			//console.log(total)
		 	let list = array.map((item) =>  "<p><span>" + item[0] + "</span>: " + (parseInt((item[1]/total) *100 )) +"%</p>").join(" ") ;
